...
class VmtraceParserGlobal:
    ...
    def parseLogsGlobal(self, contractAddress: str, txHash: str, trace):
        ...
        for ii in range(len(trace['structLogs'])):
            if "stack" in trace['structLogs'][ii] and len(trace['structLogs'][ii]["stack"]) > 0 and \
                isinstance(trace['structLogs'][ii]["stack"][-1], str):
                
                if len(trace['structLogs'][ii]["stack"]) > 0 and not ( \
                    isinstance( trace['structLogs'][ii]["stack"][0], str) and \
                        trace['structLogs'][ii]["stack"][0].startswith("0x")):

                    trace['structLogs'][ii]["stack"] = [hex(int(x)) for x in trace['structLogs'][ii]["stack"]]
        ...
        while ii < len(structLogs) - 1:
            ...
            #  calls a method in another contract, using the storage of the current contract
                ...
                temp = None
                if len(self.contractAddressStack) > 0:
                    temp = self.contractAddressStack[-1]

                infoDict = {"type": "delegatecall", "structLogsStart": ii,  "gas": gas, "addr": addr, "msg.sender": msgSender, \
                            "retOffset": retOffset, "retLength": retLength, "proxy": temp, "Raw calldata": calldata}
            ...
            # print calldatasize
                ...
                if len(self.calldataStack) == 0:
                    pass
                elif "calldatasize" not in self.calldataStack[-1]:
            ...
            # print calldataload
                ...
                if len(self.calldataStack) == 0:
                    pass
                elif "calldata" in self.calldataStack[-1] and not self.calldataStack[-1]["fixed"]:
...
def analyzeOneTxGlobal(benchmark, block, txHash, lock, isDecode = False, decodeAddresses = []):
    path = SCRIPT_DIR + "/../Benchmarks_Traces/CrossContract/{}/Txs/{}.json.gz".format(benchmark, txHash)
    ...
    try:
        ...
        return
        # fe = fetcher()
        # fe.storeTraceCross(benchmark, txHash, FullTrace=False)
        # trace = readCompressedJson(path)
        # metaTraceTree = p.parseLogsGlobal(None, txHash, trace)
    ...
    if isDecode:
        try: 
            ...
            metaTraceTree.decodeStorage(structLogs, decodeAddresses)
        except Exception as e:
            printed = "\nDecode Storage Error: " + str(e)
            printed += "\nbenchmark:  " + benchmark
            printed += "  block: " + str(block)
            printed += "  txHash: " + txHash
            print(printed)
    ...
    folder_path = SCRIPT_DIR + "/cache/{}/".format(benchmark)

    if isDecode:
        folder_path = SCRIPT_DIR + "/cache/{}_decoded/".format(benchmark)
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)

    cache_path = folder_path + "{}.json.gz".format(block)


    # print(metaTraceTree)

    # print(cache_path, "written")
    if not os.path.exists(cache_path):
        ...
        with lock:
            old_metaTraceTree = readCompressedJson(cache_path)
            old_metaTraceTree[txHash] = metaTraceTree
            # probably we need to lock the file
            writeCompressedJson(cache_path, old_metaTraceTree)


    # print(metaTraceTree)
    # print("decoding storage takes: ", end, " seconds")

    
    # new_metaTraceTree = readCompressedJson(cache_path)
    # print("New metaTraceTree: ", new_metaTraceTree[txHash])
