...
class CrawlTrueBlocks:
    ...
    def Contract2TxHistoryBlockIndex(self, contractAddress: str, firstLastBlock = None) -> list:
        """Given a contract address, return a list of (block, blockIndex) tuples"""
        self.Contract2TxHistoryWarmUp(contractAddress)
        command = None

        if firstLastBlock is None:
            command = "chifra list --fmt json " + contractAddress
        else:
            command = "chifra list --fmt json  --first_block " + str(firstLastBlock[0]) + \
                " --last_block " + str(firstLastBlock[1]) + "  " + contractAddress
        ...
        try:
            ...
            return self.Contract2TxHistoryBlockIndex(contractAddress, firstLastBlock)
