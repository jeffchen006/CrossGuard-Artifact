...
class CrawlQuickNode:
    ...
    def Tx2Receipt(self, Tx: str) -> dict:
        ...
        if "logsBloom" in receiptJson:
            del receiptJson["logsBloom"]
        if "logs" in receiptJson:
            del receiptJson["logs"]
        receipt2 = self.get_w3().eth.get_transaction(Tx)
        receiptJson2 = toDict(receipt2)

        finalReceiptJson = {"transactionHash": Tx}
        for key in [ "blockNumber", "transactionIndex", \
             "contractAddress", "from", "to", "status", "type", \
             "gasUsed", "cumulativeGasUsed", "effectiveGasPrice", "input", "value"]:
            if key in receiptJson:
                finalReceiptJson[key] = receiptJson[key]
            if key in receiptJson2:
                if key in receiptJson:
                    # print(key)
                    # print("receipt", receipt[key])
                    # print("receipt2", receipt2[key])
                    assert finalReceiptJson[key] == receiptJson2[key]
                else:
                    finalReceiptJson[key] = receiptJson2[key]

            if key not in finalReceiptJson:
                print(key)
                print(receiptJson)
                print(receiptJson2)
                # print(finalReceipt)
        ...
        _save_transaction_receipt(finalReceiptJson, self.cur, Tx, self.conn)
        return finalReceiptJson
...
def main():
    ...
    cq = CrawlQuickNode()
    tx = "0xee5cf2dbcf8d0d55a57ccd36f511e59303545468b6391657be787c01c1f42fd0"
