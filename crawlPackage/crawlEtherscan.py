...
class CrawlEtherscan:
    ...
    def Contract2Sourcecode(self, contractAddress: str) -> str:
        ...
        if receiptJson is not None and not isinstance(receiptJson, str):
            return receiptJson
        GETrequest = 'https://api.etherscan.io/v2/api?module=contract'\
            ...
            if function['type'] == 'function':
                ...
                if "stateMutability" in function:
                    if (function["stateMutability"] == "view" or function["stateMutability"] == "pure"):
                        functionSigMap[functionSelector] = (function['name'], [], [], True)
                    else:
                        functionSigMap[functionSelector] = (function['name'], [], [], False)
                else:
                    functionSigMap[functionSelector] = (function['name'], [], [], False)
    ...
    def Contract2Bytecode(self, contractAddress: str) -> str:
        ...
        GETrequest = 'https://api.etherscan.io/v2/api?module=proxy'\
            '&action=eth_getCode'\
            '&address={}'\
            '&tag=latest'\
            '&apikey={}'.format(contractAddress, self.getEtherScanAPIkey())
    ...
    def Contract2Deployer(self, contractAddress: str) -> str:
        ...
        if response['status'] == '0' and response['message'] == 'No data found':
            ...
            return None
        elif response['status'] == '0' and response['message'] == 'NOTOK' and "Max calls per sec rate limit reached (2/sec)" in response['result']:
            time.sleep(1)
    ...
    def Contract2DeployTx(self, contractAddress: str):
        ...
        if response['status'] == '0' and response['message'] == 'No data found':
            ...
            return None
        elif response['status'] == '0' and response['message'] == 'NOTOK' and "Max calls per sec" in response['result']:
            time.sleep(1)
            return self.Contract2DeployTx(contractAddress)
        else:
            # print(response)
    ...
    def Tx2Status(self, Tx: str) -> int:
        ...
        GETrequest = 'https://api.etherscan.io/v2/api'\
            '?module=transaction'\
    ...
    def Tx2Receipt(self, Tx: str) -> dict:
        ...
        GETrequest = 'https://api.etherscan.io/v2/api?chainid=1'\
            ...
            '&apikey={}'.format(Tx, self.getEtherScanAPIkey())
        response = requests.get(GETrequest).json()
        if "result" not in response or ("Max" in response["result"] and "rate limit" in response["result"]):
            time.sleep(1)
        ...
        if "status" not in receiptJson or receiptJson["status"] is None:
            ...
            # print("receriptJson: ", receiptJson)
...
def main():
    ...
    # tx = "0xf4dbe0dd1cc6700691550702460ce96c37b66fecc4534898c6d90f6dc0c4e2e6"
    # receipt = ce.Tx2Receipt2(tx)
    # print(receipt)
    ...
    functionSigMap = ce.Contract2funcSigMap("0xbf85bbc54e5107b47fe8c7ed8d8d9f4020fe706e")
