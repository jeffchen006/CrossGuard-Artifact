...
from parserPackage.parser import VmtraceParserGlobal
...
def categoryContract2ExecutionTxPaths(category: str, contract: str) -> list:
    filePath = SCRIPT_DIR + "/../constraintPackage/ReFetchedTxs/{}.pickle".format(contract.lower())
    txList = readDataSource(filePath)[0]
    pathList = []
    for txHash in txList:
        path = SCRIPT_DIR + '/{}/Txs/{}/{}.json.gz'.format(category, contract, txHash)
        pathList.append(path)
