...
class vyperAnalyzer: 
    ...
    def contract2Sourcecode(self, contractAddress: str):
        ...
        if "vyper" not in results["CompilerVersion"]:
            return None, None
        if len(results) > 1:
            sys.exit("vyperAnalyzer: cannot handle multiple Vyper contracts in one contract address {}".format(contractAddress))
    ...
    def handleSourceAndVersion(self, contractAddress: str) -> list:
        ...
        if EVMVersion != "Default":
            raise ValueError("vyperAnalyzer: cannot handle non-default EVM version for now")
