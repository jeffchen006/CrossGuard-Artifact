...
class fetcher:
    ...
    def storeTraceCross(self, incident: str, Tx: str, FullTrace: bool = False):
        """Given a tx hash, store the trace data"""
        result_dict = self.getTrace(Tx, FullTrace=FullTrace)
        SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
        path = SCRIPT_DIR + "/../Benchmarks_Traces/CrossContract/{}/Txs/{}.json.gz".format(incident, Tx)
        writeCompressedJson(path, result_dict)


def main():
    fe = fetcher()
    # fe.storeTraceCross("Yearn1_interface", "0x189f78ae203cc584d8fd054fb931604bc0d13ac71c8ce225eed167fcf9d3a05b", FullTrace=False)
